...
function txGetUnspent()
{
    ...
    var url = 'https://explorer3.mfcoin.net/api/addr/' + addr + '/utxo';
...
{
    if ( text.substring(2, 6)!="txid" )
    {
            rush.counter = 0;
            setMsg("There seems to be a problem with building the transaction. This in no way affects the safety of your coins.");
...
function txSend(hex)
{
    ...
    url = 'https://explorer3.mfcoin.net/api/tx/send';
    postdata = 'rawtx=' + hex;
...
function txRawSign (json) {
    json = JSON.parse(json);
    var prevtxs = [];
    for(var i = 0; i < json.hex.vin.length; i++) {
        prevtxs.push ({ "txid" : json.hex.vin[i].txid, "vout" : json.hex.vin[i].vout, "scriptPubKey": json.hex.vout[ json.hex.vout.length-1 ].scriptPubKey.hex, "redeemScript":""  });
    }

    url = 'https://explorer3.mfcoin.net/api/tx/signraw';
    postdata = 'hex=' + rush.txRawUnsigned + "&prevtxs=" + JSON.stringify(prevtxs) + "&privkey=" + rush.txSec;
    ajax(url, txSend, postdata);
    return false;
  }

function txRawDecode (hex) {
    url = 'https://explorer3.mfcoin.net/api/tx/rawdecode';
    rush.txRawUnsigned = JSON.parse(hex).hex;
    postdata = 'hex=' + rush.txRawUnsigned;
    ajax(url, txRawSign/*txSent*/, postdata);
    return false;
}

function txRawCreate() {

    var ins=[], outs=[];
    txJSON = JSON.parse(rush.txJSON);
    //var ins = JSON.stringify({txid: JSON.parse(txJSON).in[0].prev_out.hash, vout: Number(JSON.parse(txJSON).in[0].prev_out.n)});
    for(var i = 0; i < txJSON.in.length; i++) {
        ins.push ({ txid: txJSON.in[i].prev_out.hash, vout: Number(txJSON.in[i].prev_out.n)});
    }
    outs = {[rush.txDest] : parseFloat(rush.txAmount)};
    var change = parseFloat((rush.balance-rush.txAmount-rush.txFee).toFixed(8));
    if ( rush.txDest == rush.address ) change = parseFloat(rush.balance-rush.txFee);
    if ( change >0 ) outs[[rush.address]] = change;
    //if ( rush.txDest == rush.address ) outs = { [rush.address] : parseFloat(rush.txAmount), [rush.address] : change };

    url = 'https://explorer3.mfcoin.net/api/tx/createraw';
    postdata = 'ins=' + JSON.stringify(ins) + '&outs=' + JSON.stringify(outs);
    //console.log(postdata);
    ajax(url, txRawDecode, postdata);
    return false;
}
        ...
        if (entroMouse.generating)
        {
            ...
            if ( entroMouse.count % 10 == 1 )
            {
                ...
                else
                {
                    ...
                    {
                        $("#tapBox, #passwordCheckBox, #passBox, #recovery").hide();
...
$(document).ready(function ()
{
    ...
    */
    ...
    if (code.length > 25)
    {
        ...
        {
            $(".progress, #tapBox, #passwordCheckBox, #passBox, #recovery").hide();
