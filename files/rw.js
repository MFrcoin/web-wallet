rush = window.rush = {
    ...
    "useFiat2": false,
    "firstTime":false,
    "currencyOptions": ["CNY","EUR","GBP","JPY","RUB","USD","UAH","BTC","LTC","ETH","KZT"],
    ...
    "open": function ()
    {
        ...
        $("#address").html("<a href=\"https://explorer.mfcoin.net/address/"+this.address+"\" target=\"_blank\">"+this.address+"</a>");
        $("#explorerlink").html("<a href=\"https://explorer.mfcoin.net/address/" + this.address + "\" target=\"_blank\"></a>");

        $(".qrimage").attr("src", "https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=MFCoin%3A" + this.address + "&chld=H|0");

        rush.getBalance();

        url = "https://wallet2.mfcoin.net/?z=" + ( Math.floor(Math.random() * 9999999) + 1 ) + "#" + rush.passcode + "&{CODE}";
    ...
    "send": function ()
    {
        ...
        if (this.encrypted)
        {
            ...
            passcode = passcode.toString(CryptoJS.enc.Utf8);
    ...
    "getHistory": function ()
    {
         var url = "https://explorer3.mfcoin.net/api/txs/?address=" + this.address;
		...
		//console.log(msg);
            ...
            {
                txTime = moment( msg.txs[i].time*1000 ).format( "D MMM YYYY [<span class='time'>]hh:mm[</span>]" );
                ...
                $("#txTable tbody").append( '<tr><td>' + txTime + '</td><td class="hidden-sm hidden-xs"><a href="https://explorer.mfcoin.net/tx/' + msg.txs[i].txid + '" target="_blank" >' + msg.txs[i].txid.substring(0,30) + '...</a></td><td class="hidden-sm hidden-xs">' + confirms + '</td><td>' + tot + '</td></tr>' );
            }
            ...
            $("#txTable tbody tr td:nth-child(4)").each( function ( i )
            {
                if ( $(this).html() > 0 )
                {
                    $(this).css({color: "green", "text-align":"right", "padding-right": "30px"});
                }
                else
                {
                    $(this).css({color: "red", "text-align":"right", "padding-right": "30px"});
    ...
    "getUnconfirmed": function ()
    {
        ...
        var url = "https://explorer3.mfcoin.net/api/addr/" + this.address;
    ...
    "getFiatPrice": function ()
    {
        ...
        $.ajax({
            type: "GET",
            url: "https://explorer.mfcoin.net/api/stats/cmcdata",
            contentType: "application/json",
            dataType: 'json',

        }).done(function (result) {
			       rush.price = parseFloat((result.data[3837].quote.USD.price).toFixed(4));
             $("#price").html(rush.getFiatPrefix() + rush.price + " / MFC").show();
    ...
    "txComplete": function ()
    {
        ...
        $("#btcBalance").html( rush.balance - rush.txAmount - rush.txFee );
...
function ajax(url,success,data) {
    ...
    xhr.send(data);
    xhr.onerror = function() { console.console.log(xhr.statusText); };
}
